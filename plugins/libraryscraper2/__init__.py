...
class LibraryScraper2(_PluginBase):
    # 插件名称
    plugin_name = "自用媒体库刮削"
    # 插件描述
    ...
    # 作者主页
    author_url = "https://github.com/Griselda-droid"
    # 插件配置项ID前缀
    plugin_config_prefix = "LibraryScraper2_"
    # 加载顺序
    plugin_order = 7
    # 可使用的用户级别
    user_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._enabled or self._onlyonce:
            ...
            if self._onlyonce:
                logger.info(f"自用媒体库刮削服务，立即运行一次")
                self._scheduler = BackgroundScheduler(timezone=settings.TZ)
                self._scheduler.add_job(func=self.__LibraryScraper2, trigger='date',
    ...
    def __LibraryScraper2(self):
        ...
        for path in paths:
            ...
            if not scraper_path.exists():
                logger.warning(f"自用媒体库刮削路径不存在：{path}")
                continue
